...
![Figure 2: Illumina cluster generation and sequence imaging. Source: @illumina_image](images/illumina_method.png){width=100%}
...
![Figure 5: Hifi read length distribution (before consensus calling) from a 20kbp size selected library on the Sequel II System. Source: @pacbio_smrt ](images/hifi_readlengths.jpg){width=70%}
...
![Figure 6: PacBio read length distribution of a 35 kb size selected library. Source: @pacbio_smrt](images/pacbio_longread_lengthdist.jpg){width=70%}
...
![**Figure 7: 1D vs 1D^2^ nanopore reads.** For a 1D read, only the template strand (blue) is threaded through the nanopore (grey) by the molecular motor protein (green). For 1D^2^ reads, both the template and complement strand (orange) are sequenced by tethering the complement strand to the membrane. Source: Adapted from @delannoy_2017](images/nanopore_1D2_edit.png){width=60%}
...
![Figure 9: Cumulative Contig Length. Source: Adapted from @chakraborty_2016](images/cumulative_length_dist.png){width=80%}
