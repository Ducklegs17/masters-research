...
The most recent iteration of the PacBio sequencing platform is Sequel II. Based on a quote from DNA Link, we could expect approximately 20Gbp of Hifi reads from a single 8M SMRT Cell on the Sequel II at a cost of ~$8000 AU per cell plus \$1100 AU for library prep which equates to ~\$0.46 AU per Mbp. 
...
Working on the assumptions that 1D^2^ reads are simply 1D reads with two fold coverage and that 1D^2^ sequencing throughput is similar to 1D throughput, they would cost approximately \$0.092 per Mbp. 

```{r 1d2nan, fig.width=8,echo=FALSE,fig.cap="\\label{fig:1d2nan}1D vs $1D^2$ nanopore reads. For a 1D read, only the template strand (blue) is threaded through the nanopore (grey) by the molecular motor protein (green). For $1D^2$ reads, both the template and complement strand (orange) are sequenced by tethering the complement strand to the membrane to increase the likelihood that it will be sequenced directly after the template strand.",out.extra='', fig.pos='H'}
library(png)
library(grid)
...
```{r cumulative, fig.width=7, echo=FALSE, fig.cap="\\label{fig:cumulative}A Cumulative Length Plot gives a detailed breakdown of the distribution of assembly contigs where the height of the right most point indicates the total assembly size. Here, the blue line represents an assembly comprised primarily of longer contigs while the red line shows an assembly of the same length but comprised of much shorter contigs. In a perfect assembly, the number of contigs is equal to the number of chromosomes and the plateau is at the total genome size.",out.extra='', fig.pos='H'}
...
However, BUSCO should not be used blindly. A polyploid assembly, if successfully resolved into haplotypes, would be expected to have multiple copies of each gene and so would score very poorly in a BUSCO assessment.  

## LTR Retrotransposon content

Another method for assessing the completeness of an assembly (primarily in plants) is the LTR Assembly Index (LAI) [@ou_2018_lai]. Given that LRT Retrotransposons comprise a large portion of most plant genomes [@ou_2018] and that they are notoriously difficult to assemble due to their repetitive nature, a measure of assembly continuity of these regions is warranted [@ou_2018_lai]. Ou, Chen and Jiang noted that more complete assemblies had a higher proportion of intract LTR elements when compared with less complete genomes (2018).To leverage this fact, they used LTR_retriever [@ou_2018_retriever] to first identify all LTR elements, identified all intact LTR elements and finally applied a transformation to estimate LAI (see Equation 1 below). LAI values less than 10 indicate draft genomes, values between 10 and 20 are reference quality and LAI scores over 20 indicate gold standard assemblies [@ou_2018_lai]. 
...
Figures \ref{fig:olc} and \ref{fig:dbg} above give a basic understanding of the algorithms underlying the OLC and dBG methods respectively but it is important to realise that there is additional complexity in implementations of these algorithms due to sequencing errors, repetitive regions, and sequencing heterozygous genomes or polyploids, among others. With respect to computational cost, the OLC algorithm is generally slower than the dBG due, in part, to the explicit all-to-all pairwise comparison of reads. The OLC algorithm is also slower due to the multiple sequence alignment required for generating the consensus sequence. This is a Hamiltonian path problem which is considered to be NP Complete (meaning that it can only be solved in polynomial time and hence, that the time required to solve this problem increases rapidly with the problem size) while the dBG algorithm uses an Eulerian path which is can be solved in linear time. In summary, the OLC algorithm results in superior quality for both long read assemblies but is computationally very expensive for short read assemblies due to the all-against-all comparisons. Because of this, the dBG algorithm is often used for short read assemblies and for the assembly of very large genomes [@li_2012].  

## Assembly Methods

As mentioned earlier, the addition of at least some long reads to an assembly results in an increase in assembly quality and so only assembly methods using long reads in some manner will be considered. 

When Single Molecule Sequencing was first developed, existing tools were unable to cope with the resulting very long, low quality reads. To get around this, researchers introduced algorithms to correct errors at the beginning of the workflow (polishing the long input reads) so that tools originally designed for Sanger Sequencing reads could be repurposed [@ye_2016]. Tools utilsing the OLC algorithm were of particular interest as the OLC method appears to result in higher quality assemblies [@cherukuri_2016] with higher coverage, larger average contig lengths and less contigs. This method of error correcting erraneous long reads before assembly is still commonly used [@goodwin_2015] with Illumina reads often being used for polishing errors out of Nanopore reads or PacBio CLRs. Other assembly methods include using long reads as scaffolds for the alignment of short reads [@ye_2016] and using short reads to polish errors from a long read assembly [@nanoporetech_2016]. Long read only assemblies are also possible. 

@demaio_2019 found that ONT + Illumina hybrids led to better assemblies than PacBio + Illumina hybrids in complex bacteria and this is likely due to the greater mean length of Nanopore reads (mean read length of ~14000 bp compared with ~7000bp for PacBio). They also found that using long reads as scaffolds and Illumina reads for read error correction was more effective than simply error correcting a long read assembly with Illumina reads. In the same vein, @weirather_2017 found that both Nanopore and PacBio transcriptome analyses could benefit from a hybrid approach. They also provided a comparison of assembly concordance for a selection of different assemblies (see Figure \ref{fig:hifi_discordance} below for details). This figure indicates that, of the four options graphed, PacBio Hifi reads result in the most concordant assembly while the nanopore only assembly is the least concordant. 

```{r hifi_discordance, echo=FALSE,fig.cap="\\label{fig:hifi_discordance}Assembly discordance for ONT, ONT+Illumina, PacBio CLR and PacBio Hifi assemblies. The PacBio CCS (Hifi) assembly has the highest concordance with the reference genome while the ONT only assembly has the lowest concordance.",out.extra='', fig.pos='H',fig.asp=0.7}
img <- readPNG("images/hifi_discordance_graph_edit.png")
 grid.raster(img)
```

## Resources Required to Generate a Genome Assembly

Figure \ref{fig:flowchart} below details the essential steps necessary for generating a de novo assembly as well as some of the costs associated with each step. Managing these costs while keeping the goal of the project in mind is both essential and difficult. From Figure \ref{fig:flowchart} it can be seen that time is listed as a cost component of all three steps in the process. While this isn't necessarily a direct cost in the way that person hours are, it is an imporant consideration if you need a genome assembly as quickly as possible. The inclusion of Person Hours for both the Sequence Assembly and Quality Assessment stages is also purposeful as the time spent working out how to implement tools and thoroughly assessing assembly quality can be a significant cost to a project. While some of these costs are easily predicted, others are less so and can catch the unwary by surprise. 

```{r flowchart, echo=FALSE,fig.cap="\\label{fig:flow}Where does the money go? Costs associated with steps in generating a de novo assembly.",out.extra='', fig.pos='H',fig.asp=0.35}
library(png)
library(grid)
...
With respect to the sequencing platforms investigated here, there are major differences in read quality, error profiles, read lengths and cost. Illumina platforms generate high quality short reads at a very low cost while Nanopore reads are very long (often > 100kbp) and low quality but at a relatively low price. PacBio offers two different sequencing options; very long (up to 100 kbp), low quality reads (CLR) or shorter (~15kbp), very high quality reads (Hifi reads). De novo genome assembly using reads from these platforms can be achieved in many different ways with a range of different outcomes depending on coverage, read length, read error profile, the assembly algorithms used, and assembly methods, among other things. Finding the way through this quagmire can be challenging which brings us to the purpose of this project: to provide researchers looking to produce a de novo genome assembly with some guidance in selecting appropriate sequencing platforms and assembly methods to meet their budget, time and quality requirements using their available computing resources. This will include extensive benchmarking of assemblies created from different datasets and coverage levels as well as detailed quality analysis of the resulting assemblies.

Based on the literature, the following data combinations have been selected for de novo assembly benchmarking using the rice genome Minghui 63: 

1) PacBio Hifi-only assembly
2) Nanopore only assembly
3) Nanopore + Illumina using Illumina for error correction of reads prior to assembly
4) PacBio CLR + Illumina using Illumina for error correction of reads prior to assembly 

To ensure reproducibility, a workflow management system will be used and will be made available to the public at the conclusion of the project so that the workflows investigated are able to be re-implemented.   
