...
Pacific Biosciences made CCS read technology available to the public in 2019 [@wenger_2019]. 
 and the length of these reads is limited by the lifetime of the DNA polymerase.

is SMRT sequencing on the Sequel II is able to produce long CLR reads (??kb) but with a lower quality (raw read error rate of 13-15% [@weirather_2017]) than provided by illumina or Sanger sequencing techniques. However, while these reads are of lower quality than from previous technologies, the error distribution is random [@eid_2009] Apart from PacBio, Oxford Nanopore is the other major provider of long, lower quality reads. 
...
PacBio has also recently introduced Hifi reads to the market, or circular Consensus Sequencing (CSS), which boasts sequences that are both long and accurate. The process is very similar to normal PacBio SMRT sequencing except that hairpin adapters are ligated to both ends of DNA fragments to allow the DNA polymerase to travel around a circular template molecule multiple times and hence, to generate a consensus sequence (see Figure 2 below) [@wenger_2019]. By selecting for fragments of approximately 15kb in length (using SageELF for separation into 3kb fractions and Agilent 2100 BioAnalyzer to identify fractions centered around 15kb), Sequel produced an average consensus sequence length of 13.5±1.2 kb with mean read concordance (when compared with the reference) at 99.8% (compared with 99.9% for illumina reads).
