...
It's interesting to note that the first method noted above was developed primarily to overcome the fact that existing assembly algorithms (Overlap-Layout-Consensus and a DeBrujn graph) were unable to cope with long, low quality reads that were typical of third generation sequencing technology. Correcting errors in long reads at the beginning of a pipeline made it possible to use assembly algorithms originally designed for first generation sequencing data [@ye_2016]. In reviewing different de novo assembly methods, it becomes clear that there are a great number of different tools available for assembly (Canu, Falcon, wtdbg2, Peregrine, SMARTdenovo) but there are only a few different underlying algorithms for the assembly of long reads. Both Canu and Falcon are based on an Overlap-Layout-Consensus (OLC) algorithm which consists of three main steps:
