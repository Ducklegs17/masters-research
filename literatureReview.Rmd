...
ONT is the other major provider of single molecule sequencing technology. The MinION device was released in 2014 [@mikheyev_2014] and costs just $1000 AU for a starter pack. It reports data in real time beginning just two minutes after the sequencing run begins (with 10 minutes sample preparation using the Rapid Kit) and is literally pocket sized **(Reference Nanopore website here)**. ONT offers devices ranging from very small portable sequencers suitable for field work to benchtop sequencers such as PromethION (soon to be released) which runs up to 48 flow cells at any one time. All ONT devices, regardless of size, work in the same way. A single strand of DNA is threaded through a nanopore and changes in current across the pore are measured. Basecalling is then used to determine the actual sequence of the fragment (see Figure 6 below for more details on nanopore sequencing and basecalling). 

<br>
<center>

![Figure 6: Nanopore sequencing. A) The double stranded fragment of DNA is unwound by the unwinding enzyme at the top of the nanopore so that a single strand of DNA is threaded through the nanopore by a molecular motor protein (not shown). Approximately five bases occupy the pore at any one time and changes in current across the pore are measured and recorded. B) These changes in current are the raw data and require interpretation to determine the actual sequence. The raw data is event called to form a 'squiggle' which is then basecalled to generate the base sequence. Source: Adapted from  @minion_comp_requirements and @gpfrich_2018](images/nanopore_method.png){width=100%}
